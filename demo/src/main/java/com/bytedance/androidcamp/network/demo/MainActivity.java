...
public class MainActivity extends AppCompatActivity {
    ...
    public void testJSONObject(View view)
    {
        ...
        try
        {
            JSONObject TT= new JSONObject(CAT_JSON);
            String id = TT.getString("id");
            tvOut.setText(id);
        }
        catch (JSONException e)
        {
            Toast.makeText(this,"youmyfather",Toast.LENGTH_LONG).show();
        }
    }
        ...
        new AsyncTask<Object, Integer, String>()
        {
            ...
            protected String doInBackground(Object[] objects) {
                try {
                    ...
                    Response<List<Cat>> response = service.randomCat(3).execute();
                    Cat cat = (response==null)?null: response.body().get(0);
                    Cat cat1 = (response==null)?null: response.body().get(1);
                    Cat cat2 = (response==null)?null: response.body().get(2);
                    return cat.toString()+"\n"+cat1.toString()+"\n"+cat2.toString();
                } catch (IOException e) {
                    e.printStackTrace();
                }
